...
IntrospectableWatcher::IntrospectableWatcher(AllJoynBusAttachment^ busAttachment) :
...
IntrospectableWatcher::~IntrospectableWatcher()
{
    UnregisterFromBus();
}

void IntrospectableWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "org.allseen.Introspectable" };
...
void IntrospectableWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "org.allseen.Introspectable"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "org.allseen.Introspectable"),
            port);
        Added(this, args);
    }
...
void IntrospectableWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<IntrospectableWatcher>
        };
...
void IntrospectableWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_IntrospectableIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &IntrospectableWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void IntrospectableWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void IntrospectableWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
